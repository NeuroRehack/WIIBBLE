...
- **Purpose:**  
  The `report.py` script converts a recorded session (`.csv`) and its posturographic feature summary (`features_*.json`) into an interactive HTML document.
- **Inputs:**  
  - Raw or resampled CSV (`recordings/recording_...csv`)
  - Feature JSON generated by analysis pipeline
- **Workflow:**
  1. Reads CSV for patient/session details.
  2. Loads JSON feature set: over 80 derived posturographic measures.
  3. Plots using Plotly/Jinja2 according to standardized research/clinical references (see `VISUALISATION_REFERENCES.md`).
  4. Embeds:
      - Sway path (+confidence ellipse), Time Series (ML/AP), Velocity, Power Spectrum, Diffusion, and Spatial Density.
      - Features table.
  5. Each section gets a **neutral caption** (no interpretation).
- **Output:**  
  - HTML file with all visuals and numbers, suitable for direct clinical use, audit, or archiving.

- **CLI usage example:**
  ```powershell
  uv run python report.py recordings/recording_YYYYMMDD_HHMMSS.csv --features recordings/features_YYYYMMDD_HHMMSS.json
  ```
- **Note:**  
  Reports are only as complete as their inputs, missing the JSON, only a partial report is created.
