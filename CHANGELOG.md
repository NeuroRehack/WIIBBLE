...
### Added
- Compile-time product profiles (`WIIBBLE_PROFILE=full|lite` and per-feature env flags) so Thrive and in-app session reports can be omitted from Settings, the main Nuitka import graph, and companion exe builds
- Manual [Build (develop)](.github/workflows/build-develop.yml) workflow input to produce a full or lite Windows installer on demand
